...
import { CLAUDE_PERMISSION_MODES } from '@orbix/protocol/modes'
...
${chalk.bold('Usage:')}
  ...
  orbix auth              Manage authentication
  orbix codex             Start Codex mode
  orbix cursor            Start Cursor Agent mode
  orbix opencode          Start OpenCode ACP mode
  orbix resume [id]       Resume an existing ORBIX session locally
  orbix mcp               Start MCP stdio bridge
  orbix connect           (not available in direct-connect mode)
  orbix notify            (not available in direct-connect mode)
  orbix hub               Start the API + web hub
  orbix hub --relay       Start with public relay
  orbix server            Alias for orbix hub
  orbix runner            Manage background service that allows
                            to spawn new sessions away from your computer
  orbix doctor            System diagnostics & troubleshooting

${chalk.bold('Examples:')}
  orbix                    Start session (will prompt for token if not set)
  orbix auth login         Configure CLI_API_TOKEN interactively
  orbix --yolo             Start with bypassing permissions
                            orbix sugar for --dangerously-skip-permissions
  orbix auth status        Show direct-connect status
  orbix doctor             Run diagnostics

${chalk.bold('orbix supports ALL Claude options!')}
  Use any claude flag with orbix as you would with claude. Our favorite:
...
`)
        ...
        logger.debug('Ensuring orbix background service is running & matches our version...')

        if (!(await isRunnerRunningCurrentlyInstalledHappyVersion())) {
            logger.debug('Starting orbix background service...')
        ...
        try {
            ...
            if (
                ...
                console.error(chalk.yellow('Unable to connect to ORBIX hub'))
                ...
                console.error(chalk.gray('  Fix: run `orbix auth logout`, or set a separate ORBIX_HOME per namespace.'))
                ...
                console.error(chalk.gray('  Run: orbix auth login'))
            } else {
