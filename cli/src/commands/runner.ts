...
export const runnerCommand: CommandDefinition = {
    ...
    run: async ({ commandArgs }) => {
        ...
        console.log(`
${chalk.bold('orbix runner')} - Runner management

${chalk.bold('Usage:')}
  orbix runner start              Start the runner (replaces existing runner)
  orbix runner stop               Stop the runner (sessions stay alive)
  orbix runner status             Show runner status
  orbix runner list               List active sessions
...
${chalk.bold('Options:')}
  ...
  If you want to kill all orbix related processes run 
  ${chalk.cyan('orbix doctor clean')}

${chalk.bold('Note:')} The runner runs in the background and manages Claude sessions.
Running ${chalk.cyan('orbix runner start')} stops any existing runner first so new flags and environment variables take effect.

${chalk.bold('To clean up runaway processes:')} Use ${chalk.cyan('orbix doctor clean')}
`)
    }
}
