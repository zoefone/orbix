...
describe('resumeCommand', () => {
    ...
    it('resumes a Codex target by ORBIX session id', async () => {
        ...
        await resumeCommand.run(createContext(['orbix-session-1']))

        expect(handoffSessionToLocalMock).toHaveBeenCalledWith('orbix-session-1')
    ...
    it('rejects an active Gemini target before handoff (no longer supported, leaves running session alone)', async () => {
        ...
        getLocalResumeTargetMock.mockResolvedValue({
            sessionId: 'orbix-session-gemini',
            flavor: 'gemini',
            directory: '/tmp/project',
            machineId: 'machine-1',
            active: true,
        ...
        try {
            await expect(resumeCommand.run(createContext(['orbix-session-gemini']))).rejects.toThrow('process.exit:1')
    ...
    it('fails before launching when the target belongs to another machine', async () => {
        ...
        try {
            await expect(resumeCommand.run(createContext(['orbix-session-3']))).rejects.toThrow('process.exit:1')
    ...
    it('resumes an inactive local target even when controlledByUser is sticky', async () => {
        ...
        try {
            await resumeCommand.run(createContext(['orbix-session-4']))

            expect(exitSpy).not.toHaveBeenCalled()
            expect(consoleErrorSpy).not.toHaveBeenCalled()
            expect(handoffSessionToLocalMock).not.toHaveBeenCalled()
            expect(runClaudeMock).toHaveBeenCalledWith(expect.objectContaining({
                existingSessionId: 'orbix-session-4',
                resumeSessionId: '11111111-1111-4111-8111-111111111111'
            }))
        } finally {
            consoleErrorSpy.mockRestore()
    ...
    it('resumes a Pi target with effort', async () => {
        ...
        await resumeCommand.run(createContext(['orbix-session-pi']))

        expect(handoffSessionToLocalMock).not.toHaveBeenCalled()
        expect(runPiMock).toHaveBeenCalledWith({
            existingSessionId: 'orbix-session-pi',
            workingDirectory: '/tmp/project',
    ...
    it('keeps the non-TTY fallback and asks for an explicit session id', async () => {
        ...
        try {
            ...
            expect(consoleLogSpy).toHaveBeenCalledWith(expect.stringContaining('orbix-session-1'))
            expect(consoleErrorSpy).toHaveBeenCalledWith(expect.any(String), 'Run: orbix resume <session-id>')
        } finally {
            Object.defineProperty(process.stdin, 'isTTY', {
                configurable: true,
                value: originalIsTTY
