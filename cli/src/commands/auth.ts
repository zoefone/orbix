...
export async function handleAuthCommand(args: string[]): Promise<void> {
    ...
    if (subcommand === 'status') {
        ...
        console.log(chalk.gray(`  ORBIX_API_URL: ${configuration.apiUrl}`))
        ...
        if (!hasToken) {
            ...
            console.log(chalk.gray('    2. Read ~/.orbix/settings.json on the server'))
            console.log(chalk.gray('    3. Ask your server administrator (if token is set via env var)'))
            console.log('')
            console.log(chalk.gray('  Then run: orbix auth login'))
        }
        return
    }
...
function showHelp(): void {
    console.log(`
${chalk.bold('orbix auth')} - Authentication management

${chalk.bold('Usage:')}
  orbix auth status            Show current configuration
  orbix auth login             Enter and save CLI_API_TOKEN
  orbix auth logout            Clear saved credentials

${chalk.bold('Token priority (highest to lowest):')}
  1. CLI_API_TOKEN environment variable
  2. ~/.orbix/settings.json
