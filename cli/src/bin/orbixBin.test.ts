...
const binModulePath = path.resolve(process.cwd(), 'bin/orbix.cjs');
const {
    formatCommand,
    isSupportedPlatform,
    normalizeExecError,
...
describe('orbix binary launcher error reporting', () => {
    it('formats command with shell-safe JSON quoting', () => {
        const command = formatCommand('/tmp/orbix', ['serve', '--name', 'my agent']);
        expect(command).toBe('"/tmp/orbix" "serve" "--name" "my agent"');
    });
    ...
    it('normalizes child process execution errors', () => {
        const normalized = normalizeExecError({
            status: 132,
            signal: 'SIGILL',
            message: 'Command failed: /tmp/orbix',
        });
    ...
    it('reports execution details before exit handling', () => {
        ...
        expect(lines).toEqual([
            'Failed to execute: "/tmp/orbix" "serve" "--port" "3000"',
    ...
    it('handles unknown failures with generic output', () => {
        ...
        const result = reportExecutionFailure({}, '/tmp/orbix', [], (line: string) => {
            lines.push(line);
        });

        expect(result).toEqual({ status: null, signal: null });
        expect(lines).toEqual(['Failed to execute: "/tmp/orbix"']);
    });
