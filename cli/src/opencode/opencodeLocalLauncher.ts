...
export async function opencodeLocalLauncher(
    ...
    // Start the orbix MCP server for change_title support (optional feature)
    let happyServer: { url: string; stop: () => void } | null = null;
    let opencodeConfigPath: string | null = null;
    try {
        const bridge = await buildOrbixMcpBridge(session.client);
        happyServer = bridge.server;
        logger.debug(`[opencode-local]: Started orbix MCP server at ${happyServer.url}`);

        // Generate opencode.json config with MCP server and instructions
        const { configPath } = ensureOpencodeConfig(opencodeConfigDir, bridge.mcpServers.orbix, TITLE_INSTRUCTION);
        opencodeConfigPath = configPath;
    } catch (error) {
        logger.debug('[opencode-local]: Failed to start orbix MCP server (change_title will be unavailable)', error);
    }
    ...
    const launcher = new BaseLocalLauncher({
        ...
        launch: async (abortSignal) => {
            ...
            env.ORBIX_OPENCODE_HOOK_URL = hookUrl;
            env.ORBIX_OPENCODE_HOOK_TOKEN = opts.hookServer.token;
    ...
    try {
        ...
        if (happyServer) {
            happyServer.stop();
            logger.debug('[opencode-local]: Stopped orbix MCP server');
        }
    }
}
