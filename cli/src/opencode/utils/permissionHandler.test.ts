...
describe('OpencodePermissionHandler plan mode', () => {
    ...
    it('still auto-approves orbix title updates in plan mode', async () => {
        const harness = createHarness(() => 'plan');
        ...
        expect(harness.respondCalls).toEqual([
            {
                sessionId: 'session-1',
                request: expect.objectContaining({ id: 'perm-title', title: 'orbix_change_title' }),
