...
const PLUGIN_FILENAME = 'orbix-hook.ts';
const PACKAGE_JSON_FILENAME = 'package.json';
const PLUGIN_PACKAGE = '@opencode-ai/plugin';
// Pinned to the major version ORBIX has been validated against (opencode
...
function buildPluginSource(hookUrl: string, token: string): string {
    ...
    return [
        '// Generated by ORBIX. Do not edit manually.',
        '',
        `const DEFAULT_HOOK_URL = ${escapedUrl};`,
        `const DEFAULT_HOOK_TOKEN = ${escapedToken};`,
        'const HOOK_URL = process.env.ORBIX_OPENCODE_HOOK_URL || DEFAULT_HOOK_URL;',
        'const HOOK_TOKEN = process.env.ORBIX_OPENCODE_HOOK_TOKEN || DEFAULT_HOOK_TOKEN;',
        '',
        ...
        '}',
        '',
        'export const OrbixHookPlugin = async () => {',
        '    return {',
        '        event: async ({ event }) => {',
...
/**
 * `rootPath` is a ORBIX-managed directory only when it lives under
 ...
 */
function isOrbixManagedDir(rootPath: string): boolean {
    ...
    // input (e.g. home=`C:\\orbix`, rootPath=`D:\\…`) makes `relative`
 ...
 * that package is resolvable from `<configDir>`. ORBIX's per-session
 * OPENCODE_CONFIG_DIR is otherwise an empty directory, so plugin discovery
 * never even attempts to load orbix-hook.ts.
 *
...
function ensurePluginRuntime(rootPath: string): void {
    if (!isOrbixManagedDir(rootPath)) {
        logger.debug(`[opencode-hook] Skipping plugin runtime materialization for non-ORBIX dir: ${rootPath}`);
        return;
    }
