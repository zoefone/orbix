/**
 * Global configuration for ORBIX CLI
 *
...
export function parseExtraHeaders(raw: string | undefined, warn: (message: string) => void = console.warn): Record<string, string> {
    ...
    try {
        ...
        if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
            warn('[WARN] ORBIX_EXTRA_HEADERS_JSON must be a JSON object. Ignoring value.')
            return {}
        }
        ...
        if (Object.keys(headers).length !== entries.length) {
            warn('[WARN] ORBIX_EXTRA_HEADERS_JSON only supports string header values. Ignoring non-string entries.')
        }

        return headers
    } catch {
        warn('[WARN] Failed to parse ORBIX_EXTRA_HEADERS_JSON. Ignoring value.')
        return {}
    }
}
...
class Configuration {
    ...
    constructor() {
        ...
        this._apiUrl = process.env.ORBIX_API_URL || 'http://localhost:3006'
        this._cliApiToken = process.env.CLI_API_TOKEN || ''
        this._extraHeaders = parseExtraHeaders(process.env.ORBIX_EXTRA_HEADERS_JSON)
        ...
        // Directory configuration - Priority: ORBIX_HOME env > default home dir
        if (process.env.ORBIX_HOME) {
            // Expand ~ to home directory if present
            const expandedPath = process.env.ORBIX_HOME.replace(/^~/, homedir())
            this.happyHomeDir = expandedPath
        } else {
            this.happyHomeDir = join(homedir(), '.orbix')
        }
        ...
        this.isExperimentalEnabled = ['true', '1', 'yes'].includes(process.env.ORBIX_EXPERIMENTAL?.toLowerCase() || '')
