/**
 * Minimal persistence functions for ORBIX CLI
 * 
 * Handles settings, encryption key, and runner state storage in ~/.orbix/ (or ORBIX_HOME override)
 */
...
interface Settings {
  ...
  cliApiToken?: string
  // API URL for server connections (priority: env ORBIX_API_URL > this > default)
  apiUrl?: string
...
export interface RunnerLocallyPersistedState {
  ...
  /**
   * Snapshot of ORBIX_DISABLE_VERSION_HANDOFF=1 at the time this runner
   * started. Lets a later `orbix runner start` invocation (from a shell where
