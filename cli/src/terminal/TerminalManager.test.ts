...
const originalTerminalShell = process.env.ORBIX_TERMINAL_SHELL
...
describe('TerminalManager Windows support', () => {
    ...
    beforeEach(() => {
        ...
        process.env.ORBIX_TERMINAL_SHELL = 'C:\\Program Files\\PowerShell\\7\\pwsh.exe'
        setPlatform('win32')
    })
    ...
    afterAll(() => {
        ...
        if (originalTerminalShell === undefined) {
            delete process.env.ORBIX_TERMINAL_SHELL
        } else {
            process.env.ORBIX_TERMINAL_SHELL = originalTerminalShell
        }
