...
import type { DeleteUploadResponse, UploadFileResponse } from '@orbix/protocol/apiTypes'
import { RPC_METHODS } from '@orbix/protocol/rpcMethods'
import type { RpcHandlerManager } from '@/api/rpc/RpcHandlerManager'
import { getErrorMessage, rpcError } from '../rpcResponses'
import { getOrbixBlobsDir } from '@/constants/uploadPaths'
...
async function getOrCreateUploadDir(sessionId?: string): Promise<string> {
    ...
    const creation = (async () => {
        try {
            const blobsDir = getOrbixBlobsDir()
...
function isPathWithinUploadDir(path: string, sessionId?: string): boolean {
    ...
    const safeKey = sanitizeFilename(sessionKey)
    const resolvedPrefix = resolve(getOrbixBlobsDir(), `${safeKey}-`)
