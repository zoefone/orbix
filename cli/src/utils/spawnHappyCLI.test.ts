...
const {
  ...
  existsSyncMock: vi.fn((path: string) => !path.includes('missing-orbix.exe')),
...
const originalInvokedCwd = process.env.ORBIX_INVOKED_CWD;
const originalCliExecutable = process.env.ORBIX_CLI_EXECUTABLE;
const originalBunExec = process.env.ORBIX_BUN_EXEC;
...
describe('spawnHappyCLI windowsHide behavior', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.resetModules();
    existsSyncMock.mockImplementation((path: string) => !path.includes('missing-orbix.exe'));
    isBunCompiledMock.mockReturnValue(false);
    projectPathMock.mockReturnValue(process.cwd());
    if (originalInvokedCwd === undefined) {
      delete process.env.ORBIX_INVOKED_CWD;
    } else {
      process.env.ORBIX_INVOKED_CWD = originalInvokedCwd;
    }
    if (originalCliExecutable === undefined) {
      delete process.env.ORBIX_CLI_EXECUTABLE;
    } else {
      process.env.ORBIX_CLI_EXECUTABLE = originalCliExecutable;
    }
    if (originalBunExec === undefined) {
      delete process.env.ORBIX_BUN_EXEC;
    } else {
      process.env.ORBIX_BUN_EXEC = originalBunExec;
    }
  });
  ...
  it('uses an inherited compiled CLI executable override when it points to an existing binary', async () => {
    ...
    process.env.ORBIX_CLI_EXECUTABLE = 'C:\\Users\\Administrator\\.orbix\\patched\\orbix.exe';
    const { getHappyCliCommand, resolveHappyCliExecutable } = await import('./spawnHappyCLI');

    const command = getHappyCliCommand(['mcp', '--url', 'http://127.0.0.1:1234/']);

    expect(resolveHappyCliExecutable()).toBe(process.env.ORBIX_CLI_EXECUTABLE);
    expect(command.command).toBe(process.env.ORBIX_CLI_EXECUTABLE);
  });

  it('falls back to a real argv0 executable before process.execPath in compiled mode', async () => {
    isBunCompiledMock.mockReturnValue(true);
    const previousCliExecutable = process.env.ORBIX_CLI_EXECUTABLE;
    delete process.env.ORBIX_CLI_EXECUTABLE;
    const previousArgv0 = process.argv[0];
    process.argv[0] = 'C:\\Users\\Administrator\\.orbix\\patched\\resume-recovery-0.17.2\\orbix.exe';
    ...
    try {
      ...
      if (previousCliExecutable === undefined) {
        ...
        process.env.ORBIX_CLI_EXECUTABLE = previousCliExecutable;
      }
    }
  });
  ...
  it('ignores an inherited compiled CLI executable override when the binary is missing', async () => {
    isBunCompiledMock.mockReturnValue(true);
    process.env.ORBIX_CLI_EXECUTABLE = 'C:\\Users\\Administrator\\.orbix\\patched\\missing-orbix.exe';
    const { getHappyCliCommand } = await import('./spawnHappyCLI');
  ...
  it('passes the resolved compiled executable to child ORBIX processes', async () => {
    isBunCompiledMock.mockReturnValue(true);
    process.env.ORBIX_CLI_EXECUTABLE = 'C:\\Users\\Administrator\\.orbix\\patched\\orbix.exe';
    const { spawnHappyCLI } = await import('./spawnHappyCLI');
    ...
    expect(command).toBe(process.env.ORBIX_CLI_EXECUTABLE);
    expect((options as SpawnOptions | undefined)?.env?.ORBIX_CLI_EXECUTABLE).toBe(process.env.ORBIX_CLI_EXECUTABLE);
  });
  ...
  it('passes invoked workspace cwd to child processes when cwd is provided', async () => {
    ...
    const options = getSpawnOptionsOrThrow();
    expect(options.env?.ORBIX_INVOKED_CWD).toBe(childCwd);
  });

  it('prefers the explicit child cwd over an inherited ORBIX_INVOKED_CWD', async () => {
    const { spawnHappyCLI } = await import('./spawnHappyCLI');
    ...
    spawnHappyCLI(['runner', 'start-sync'], {
      cwd: childCwd,
      env: {
        ORBIX_INVOKED_CWD: inheritedInvokedCwd
      },
      stdio: 'ignore'
    });
  ...
  it('keeps an existing absolute ORBIX_INVOKED_CWD when no child cwd is provided', async () => {
    const { spawnHappyCLI } = await import('./spawnHappyCLI');
    const inheritedInvokedCwd = 'C:\\workspace\\other-project';
    ...
    const options = getSpawnOptionsOrThrow();
    expect(options.env?.ORBIX_INVOKED_CWD).toBe(inheritedInvokedCwd);
  });
});
