...
function readWorktreeFromEnv(): WorktreeInfo | null {
    const basePath = process.env.ORBIX_WORKTREE_BASE_PATH?.trim();
    const branch = process.env.ORBIX_WORKTREE_BRANCH?.trim();
    const name = process.env.ORBIX_WORKTREE_NAME?.trim();
    const worktreePath = process.env.ORBIX_WORKTREE_PATH?.trim();
    const createdAtRaw = process.env.ORBIX_WORKTREE_CREATED_AT?.trim();
