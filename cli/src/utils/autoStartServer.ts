 ...
 *
 * Automatically starts the ORBIX hub when CLI is launched
 * if specific conditions are met:
 * 1. ORBIX_API_URL is not set (using default localhost:3006)
...
async function shouldAutoStartServer(): Promise<boolean> {
    // Condition 1: ORBIX_API_URL not set (using default localhost:3006)
    if (process.env.ORBIX_API_URL) {
        logger.debug('[AUTO-START] ORBIX_API_URL is set, skipping auto-start')
        return false
    }
...
export async function maybeAutoStartServer(): Promise<void> {
    try {
        ...
        console.log(chalk.gray('Starting ORBIX hub in background...'))
        ...
        if (!isReady) {
            ...
            console.log(chalk.gray('  Try running `orbix hub` manually to see errors'))
            return
        }

        console.log(chalk.green('ORBIX hub started'))
    } catch (error) {
