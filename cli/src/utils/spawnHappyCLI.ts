/**
 * Cross-platform ORBIX CLI spawning utility
 *
 * ## Background
 *
 * ORBIX CLI runs in two modes:
 ...
 *
 * This utility handles spawning ORBIX CLI subprocesses (for runner processes)
...
const ORBIX_CLI_EXECUTABLE_ENV = 'ORBIX_CLI_EXECUTABLE';
...
function resolveInvokedCwd(cwd: SpawnOptions['cwd']): string {
  ...
  const inheritedInvokedCwd = process.env.ORBIX_INVOKED_CWD?.trim();
...
export function resolveHappyCliExecutable(): string {
  const override = process.env[ORBIX_CLI_EXECUTABLE_ENV]?.trim();
...
export function getHappyCliCommand(args: string[]): HappyCliCommand {
  ...
  if (isBunRuntime) {
    ...
    // keeps working even when external tools launch ORBIX from another workspace.
    return {
  ...
  // When vitest runs under Node.js, ORBIX_BUN_EXEC can point to the bun binary so that
  // spawned CLI child processes still run under bun (which is required for TypeScript entrypoints).
  const bunExecOverride = process.env['ORBIX_BUN_EXEC']?.trim();
...
export function spawnHappyCLI(args: string[], options: SpawnOptions = {}): ChildProcess {
  ...
  // bypassing the 'orbix' wrapper that would normally be found in the shell's PATH.
  // However, we log it as 'orbix' here because other engineers are typically looking
  // for when "orbix" was started and don't care about the underlying node process
  // details and flags we use to achieve the same result.
  const fullCommand = `orbix ${args.join(' ')}`;
  logger.debug(`[SPAWN ORBIX CLI] Spawning: ${fullCommand} in ${directory}`);
  ...
  if (!compiledMode) {
    ...
    if (entrypoint && !existsSync(entrypoint)) {
      ...
      logger.debug(`[SPAWN ORBIX CLI] ${errorMessage}`);
  ...
  if (compiledMode) {
    finalEnv[ORBIX_CLI_EXECUTABLE_ENV] = spawnCommand;
    shouldSetEnv = true;
  } else {
    const invokedCwd = finalEnv.ORBIX_INVOKED_CWD?.trim();
    const hasExplicitCwd = 'cwd' in options && options.cwd !== undefined;
    finalEnv.ORBIX_INVOKED_CWD = hasExplicitCwd
