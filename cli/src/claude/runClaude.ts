...
export async function runClaude(options: StartOptions = {}): Promise<void> {
    ...
    logger.debugLargeJson('[START] ORBIX process started', getEnvironmentInfo());
    ...
    // Start ORBIX MCP server
    const happyServer = await startHappyServer(session);
    logger.debug(`[START] ORBIX MCP server started at ${happyServer.url}`);
    ...
    try {
        await loop({
            ...
            messageQueue,
            api,
            allowedTools: happyServer.toolNames.map(toolName => `mcp__orbix__${toolName}`),
