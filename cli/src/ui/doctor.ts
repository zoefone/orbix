 ...
 * for orbix CLI including configuration, runner status, logs, and links
 */
...
export function getEnvironmentInfo(): Record<string, any> {
    return {
        PWD: process.env.PWD,
        ORBIX_HOME: process.env.ORBIX_HOME,
        ORBIX_API_URL: process.env.ORBIX_API_URL,
        ORBIX_PROJECT_ROOT: process.env.ORBIX_PROJECT_ROOT,
...
export async function runDoctorCommand(filter?: 'all' | 'runner'): Promise<void> {
    ...
    if (filter === 'all') {
        ...
        console.log(`orbix CLI Version: ${chalk.green(packageJson.version)}`);
        ...
        console.log(`orbix Home: ${chalk.blue(configuration.happyHomeDir)}`);
        ...
        console.log(`ORBIX_HOME: ${env.ORBIX_HOME ? chalk.green(env.ORBIX_HOME) : chalk.gray('not set')}`);
        console.log(`ORBIX_API_URL: ${env.ORBIX_API_URL ? chalk.green(env.ORBIX_API_URL) : chalk.gray('not set')}`);
        ...
        if (hasToken) {
            ...
            console.log(chalk.gray('  Run `orbix auth login` to configure or set CLI_API_TOKEN env var'));
        }
    ...
    try {
        ...
        if (allProcesses.length > 0) {
            ...
            console.log(chalk.red('❌ No orbix processes found'));
        }

        if (filter === 'all' && allProcesses.length > 1) { // More than just current process
            console.log(chalk.bold('\n💡 Process Management'));
            console.log(chalk.gray('To clean up runaway processes: orbix doctor clean'));
        }
    } catch (error) {
