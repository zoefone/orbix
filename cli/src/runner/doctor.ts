...
/**
 * Find all ORBIX CLI processes (including current process)
 */
export async function findAllHappyProcesses(): Promise<Array<{ pid: number, command: string, type: string }>> {
  try {
    ...
    for (const proc of processes) {
      ...
      // Check if it's a ORBIX process
      const isHappyBinary = name === 'orbix' || name === 'orbix.exe' || /\borbix(\.exe)?\b/.test(cmd);
...
/**
 * Find all runaway ORBIX CLI processes that should be killed
 */
...
/**
 * Kill all runaway ORBIX CLI processes
 */
