 ...
 * - ORBIX_HOME=~/.orbix-dev-test (DIFFERENT from dev's ~/.orbix-dev!)
 * - ORBIX_API_URL=http://localhost:3006 (local orbix-hub)
...
describe.skipIf(!await isServerHealthy())('Runner Integration Tests', { timeout: 20_000 }, () => {
  ...
  it('should track session-started webhook from terminal session', async () => {
    ...
    expect(tracked.startedBy).toBe('orbix directly - likely by user from terminal');
  ...
  it('should track both runner-spawned and terminal sessions', async () => {
    // Spawn a real orbix process that looks like it was started from terminal
    ...
    if (!terminalHappyProcess || !terminalHappyProcess.pid) {
      throw new Error('Failed to spawn terminal orbix process');
    }
    ...
    expect(terminalSession.startedBy).toBe('orbix directly - likely by user from terminal');
   ...
   * 
   * This simulates what happens during `npm upgrade orbix`:
   * - Running runner has OLD version loaded in memory (configuration.currentCliVersion)
   * - npm replaces node_modules/orbix/ with NEW version files
  ...
  it('[takes 1 minute to run] should detect version mismatch and kill old runner', { timeout: 100_000 }, async () => {
    ...
    try {
      ...
      await new Promise(resolve => setTimeout(resolve, parseInt(process.env.ORBIX_RUNNER_HEARTBEAT_INTERVAL || '30000') + 10_000));
  ...
  // TODO: Test npm uninstall scenario - runner should gracefully handle when orbix is uninstalled
