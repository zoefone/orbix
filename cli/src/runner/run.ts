...
import { PERMISSION_MODES } from '@orbix/protocol/modes';
...
export async function startRunner(options: { workspaceRoots?: string[] } = {}): Promise<void> {
  ...
  let requestShutdown: (source: 'orbix-app' | 'orbix-cli' | 'os-signal' | 'exception', errorMessage?: string) => void;
  let resolvesWhenShutdownRequested = new Promise<({ source: 'orbix-app' | 'orbix-cli' | 'os-signal' | 'exception', errorMessage?: string })>((resolve) => {
  ...
  // ORBIX_RUNNER_HANDOFF_FROM_PID=<parent_pid> on the spawned child's env;
  ...
  //
  // New protocol:
  //   - Parent: spawn child with ORBIX_RUNNER_HANDOFF_FROM_PID env, then
  ...
  const handoffFromPidRaw = process.env.ORBIX_RUNNER_HANDOFF_FROM_PID;
  ...
  if (Number.isFinite(handoffFromPid) && handoffFromPid > 0) {
    ...
    if (existingState?.pid === handoffFromPid && isProcessAlive(handoffFromPid)) {
      ...
      logger.debug(`[RUNNER RUN] ORBIX_RUNNER_HANDOFF_FROM_PID=${handoffFromPidRaw} set but no matching live parent in state (state.pid=${existingState?.pid ?? 'none'}); ignoring handoff signal`);
    }
  }
  ...
  try {
    ...
    const envWebhookTimeout = Number(process.env.ORBIX_RUNNER_WEBHOOK_TIMEOUT_MS);
    ...
    // Handle webhook from ORBIX session reporting itself
    const onHappySessionWebhook = (sessionId: string, sessionMetadata: Metadata) => {
      ...
      if (existingSession && existingSession.startedBy === 'runner') {
        ...
        const trackedSession: TrackedSession = {
          startedBy: 'orbix directly - likely by user from terminal',
      ...
      try {
        ...
        if (options.token) {
          if (options.agent === 'codex') {
            ...
            const codexHomeDir = await fs.mkdtemp(join(os.tmpdir(), 'orbix-codex-'));
        ...
        if (worktreeInfo) {
          extraEnv = {
            ...extraEnv,
            ORBIX_WORKTREE_BASE_PATH: worktreeInfo.basePath,
            ORBIX_WORKTREE_BRANCH: worktreeInfo.branch,
            ORBIX_WORKTREE_NAME: worktreeInfo.name,
            ORBIX_WORKTREE_PATH: worktreeInfo.worktreePath,
            ORBIX_WORKTREE_CREATED_AT: String(worktreeInfo.createdAt)
          };
        }
        ...
        if (!happyProcess.pid) {
          ...
          const errorMessage = `Failed to spawn ORBIX process - no PID returned (${details.join('; ')})`;
        ...
        const spawnResult = await new Promise<SpawnSessionResult>((resolve) => {
          ...
          // ORBIX_RUNNER_WEBHOOK_TIMEOUT_MS for users on slow models
          // (e.g. opus[1m] --resume).
    ...
    const { port: controlPort, stop: stopControlServer } = await startRunnerControlServer({
      ...
      stopSession,
      spawnSession,
      requestShutdown: () => requestShutdown('orbix-cli'),
    ...
    // `[orbix, runner, start-sync, ...]` so slice(2) produced `['start-sync', ...]`.
    // The replacement then spawned `orbix start-sync ...`, which `resolveCommand`
    ...
    // Persisted to runner.state.json so a later `orbix runner start` from a
    // shell where the env var is NOT set can still honour the opt-out
    // (Codex review #814 [Major] - controlClient.ts:192 fix).
    const startedWithVersionHandoffDisabled = process.env.ORBIX_DISABLE_VERSION_HANDOFF === '1';
    ...
    apiMachine.setRPCHandlers({
      spawnSession,
      stopSession,
      requestShutdown: () => requestShutdown('orbix-app')
    });
    ...
    console.log('');
    console.log('Orbix runner started.');
    ...
    // 4. Write heartbeat
    const heartbeatIntervalMs = parseInt(process.env.ORBIX_RUNNER_HEARTBEAT_INTERVAL || '60000');
    ...
    const restartOnStaleVersionAndHeartbeat = setInterval(async () => {
      ...
      // unrelated to an actual npm upgrade. ORBIX_DISABLE_VERSION_HANDOFF=1
      // keeps the rest of the heartbeat (session pruning, state file
      // persistence) intact.
      if (process.env.ORBIX_DISABLE_VERSION_HANDOFF === '1') {
        if (process.env.DEBUG) {
          logger.debug('[RUNNER RUN] ORBIX_DISABLE_VERSION_HANDOFF=1 set, skipping mtime/version drift self-restart');
        }
      } else {
        ...
        if (typeof installedCliMtimeMs === 'number' &&
          ...
          // Spawn the replacement with ORBIX_RUNNER_HANDOFF_FROM_PID set so
          ...
          try {
            spawnHappyCLI(handoffArgv, {
              ...
              env: {
                ...process.env,
                ORBIX_RUNNER_HANDOFF_FROM_PID: String(process.pid)
              }
            });
          } catch (error) {
          ...
          // lock-retry window when ORBIX_RUNNER_HANDOFF_FROM_PID is set, so
    ...
    // Setup signal handlers
    const cleanupAndShutdown = async (source: 'orbix-app' | 'orbix-cli' | 'os-signal' | 'exception', errorMessage?: string) => {
...
export function buildCliArgs(
  ...
  args.push('--orbix-starting-mode', 'remote', '--started-by', 'runner');
  if (options.model) {
