# ORBIX CLI Runner: Control Flow and Lifecycle

The runner is a persistent background process that manages ORBIX sessions, enables remote control from the mobile app, and handles auto-updates when the CLI version changes.

## 1. Runner Lifecycle

### Starting the Runner

Command: `orbix runner start`
...
4. `startRunner()` performs startup:
   ...
   - Heartbeat loop: every 60s (or `ORBIX_RUNNER_HEARTBEAT_INTERVAL`) checks for version updates, prunes dead sessions, verifies PID ownership
5. Awaits shutdown promise which resolves when:
   - OS signal received (SIGINT/SIGTERM) - source: `os-signal`
   - HTTP `/stop` endpoint called - source: `orbix-cli`
   - RPC `stop-runner` invoked - source: `orbix-app`
...
The runner detects when CLI binary changes (e.g., after `npm upgrade orbix`):
...
Every 60 seconds (configurable via `ORBIX_RUNNER_HEARTBEAT_INTERVAL`):
...
|-------|---------|-------------------|
| `claude` (default) | `orbix claude` | `CLAUDE_CODE_OAUTH_TOKEN` |
| `codex` | `orbix codex` | `CODEX_HOME` (temp directory with `auth.json`) |
| `gemini` | `orbix gemini` | - |
| `opencode` | `orbix opencode` | OpenCode config (no token injection) |

### Token Authentication

When spawning a session with a token:
- **Claude**: Sets `CLAUDE_CODE_OAUTH_TOKEN` environment variable
- **Codex**: Creates temp directory at `os.tmpdir()/orbix-codex-*`, writes token to `auth.json`, sets `CODEX_HOME`
...
3. `spawnSession()`:
   ...
   - Spawns detached ORBIX process with `--orbix-starting-mode remote --started-by runner`
...
User runs `orbix` directly:
1. CLI auto-starts runner if configured
2. ORBIX process calls `notifyRunnerSessionStarted()`
3. Runner receives webhook, creates `TrackedSession` with `startedBy: 'orbix directly - likely by user from terminal'`
...
`orbix doctor` uses `ps aux | grep` to find all ORBIX processes:
- Production: matches `orbix` binary, `happy-coder`
...
- Uses local orbix-hub (http://localhost:3006)
- Separate `~/.orbix-dev-test` home directory
...
> Direct-connect note: the "hub" is `orbix-hub`, payloads are plain JSON (no base64/encryption),
...
interface RunnerState {
  ...
  shutdownSource?: 'orbix-app' | 'orbix-cli' | 'os-signal' | 'exception';
}
```
...
```json
{
  ...
  "metadata": {
    ...
    "homeDir": "/Users/john",
    "happyHomeDir": "/Users/john/.orbix",
    "happyLibDir": "/usr/local/lib/node_modules/orbix"
  },
  "runnerState": {
    "status": "running",
    "pid": 12345,
...
## 5. Mini App RPC Calls (via orbix-hub)

The Telegram Mini App calls REST endpoints on `orbix-hub` (for example `POST /api/machines/:id/spawn`).
`orbix-hub` then relays those requests to the runner via Socket.IO `rpc-request` on the `/cli` namespace.
