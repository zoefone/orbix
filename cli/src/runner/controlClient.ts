...
import { isProcessAlive, isOrbixRunnerProcess, killProcess } from '@/utils/process';
...
async function runnerPost(path: string, body?: any): Promise<{ error?: string } | any> {
  ...
  try {
    const timeout = process.env.ORBIX_RUNNER_HTTP_TIMEOUT ? parseInt(process.env.ORBIX_RUNNER_HTTP_TIMEOUT) : 10_000;
 ...
 * For instance we are not handling the case where we upgraded orbix,
 ...
 * For instance when running `orbix runner status` we can show more information.
 */
export async function checkIfRunnerRunningAndCleanupStaleState(): Promise<boolean> {
  ...
  // Verify PID is alive AND belongs to orbix (not a reused PID from another process)
  if (isOrbixRunnerProcess(state.pid)) {
    return true;
  }

  logger.debug('[RUNNER RUN] Runner PID not running or not a orbix process, cleaning up state');
...
export async function isRunnerRunningCurrentlyInstalledHappyVersion(): Promise<boolean> {
  ...
  const currentApiUrl = process.env.ORBIX_API_URL
    || settings.apiUrl
    || settings.serverUrl
  ...
  try {
    // When ORBIX_DISABLE_VERSION_HANDOFF=1 is set on the live runner (operator
    ...
    // Otherwise `orbix runner start` would kill the live runner mid-rebuild.
    //
    ...
    if (
      process.env.ORBIX_DISABLE_VERSION_HANDOFF === '1'
