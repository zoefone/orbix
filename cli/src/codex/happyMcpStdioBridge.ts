/**
 * ORBIX MCP STDIO Bridge
 *
 * Minimal STDIO MCP server exposing ORBIX tools such as `change_title` and `display_image`.
 * On invocation it forwards the tool call to an existing ORBIX HTTP MCP server
 * using the StreamableHTTPClientTransport.
 *
 * Configure the target HTTP MCP URL via env var `ORBIX_HTTP_MCP_URL` or
...
export async function runHappyMcpStdioBridge(argv: string[]): Promise<void> {
  try {
    ...
    const baseUrl = urlFromArgs || process.env.ORBIX_HTTP_MCP_URL || '';

    if (!baseUrl) {
      // Write to stderr; never stdout.
      process.stderr.write(
        '[orbix-mcp] Missing target URL. Set ORBIX_HTTP_MCP_URL or pass --url <http://127.0.0.1:PORT>\n'
      );
      process.exit(2);
    }
    ...
    async function ensureHttpClient(): Promise<Client> {
      ...
      const client = new Client(
        { name: 'orbix-stdio-bridge', version: '1.0.0' },
        { capabilities: {} }
      );
    ...
    try {
      process.stderr.write(`[orbix-mcp] Fatal: ${err instanceof Error ? err.message : String(err)}\n`);
    } finally {
      process.exit(1);
    }
  }
