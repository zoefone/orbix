...
vi.mock('./utils/buildOrbixMcpBridge', () => ({
    buildOrbixMcpBridge: async (_client: unknown, options?: unknown) => {
...
function createSessionStub(messages = ['hello from launcher test'], mode = createMode()) {
    ...
    const session = {
        path: '/tmp/orbix-update',
        logPath: '/tmp/orbix-update/test.log',
        client,
        queue,
        codexArgs: undefined,
...
describe('codexRemoteLauncher', () => {
    ...
    it('applies parent-thread orbix change_title after disabling MCP-side title writes', async () => {
