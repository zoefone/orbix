...
import { CodexCollaborationModeSchema, PermissionModeSchema } from '@orbix/protocol/schemas';
...
export async function runCodex(opts: {
    ...
    // 中文注释：当用户直接把现成的 Codex thread 导入到一个全新的 Orbix 会话时，
    // 需要在首次附着 transcript 时回放已有历史；恢复已有 Orbix 会话时则保持原来的增量模式，避免重复灌入旧消息。
