...
import type { McpServersConfig } from './utils/buildOrbixMcpBridge';

/**
 * Filter out 'resume' subcommand which is managed internally by orbix.
...
export function filterResumeSubcommand(args: string[]): string[] {
    ...
    if (args.length > 1 && !args[1].startsWith('-')) {
        logger.debug(`[CodexLocal] Filtered 'resume ${args[1]}' - session managed by orbix`);
        return args.slice(2);
    }

    logger.debug(`[CodexLocal] Filtered 'resume' - session managed by orbix`);
    return args.slice(1);
}
