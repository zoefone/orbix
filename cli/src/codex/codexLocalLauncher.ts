...
export async function codexLocalLauncher(session: CodexSession): Promise<'switch' | 'exit'> {
    ...
    // Start orbix hub for MCP bridge (same as remote mode)
    const { server: happyServer, mcpServers } = await buildOrbixMcpBridge(session.client);
    logger.debug(`[codex-local]: Started orbix MCP bridge server at ${happyServer.url}`);
    ...
    const processTranscriptPath = async (transcriptPath: string): Promise<void> => {
        ...
        const createdScanner = await createCodexSessionScanner({
            transcriptPath,
            // 中文注释：导入模式下允许 scanner 首次回放 transcript 全量内容，补齐 Codex 客户端里已有但 Orbix 还未看到的消息。
    ...
    try {
        ...
        logger.debug('[codex-local]: Stopped orbix MCP bridge server');
    }
}
