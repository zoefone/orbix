...
describe('codexMcpConfig', () => {
    describe('buildMcpServerConfigArgs', () => {
        it('builds config args for a single MCP server', () => {
            ...
            expect(args).toEqual([
                '-c', 'mcp_servers.orbix.command="orbix"',
                '-c', "mcp_servers.orbix.args=['mcp','--url','http://localhost:3000']"
            ]);
        });
        ...
        it('builds config args for multiple MCP servers', () => {
            ...
            expect(args).toContain('mcp_servers.orbix.command="orbix"');
    ...
    describe('buildDeveloperInstructionsArg', () => {
        it('builds developer instructions arg', () => {
            const instructions = 'Call functions.orbix__change_title to set title.';

            const args = buildDeveloperInstructionsArg(instructions);

            expect(args).toEqual([
                '-c',
                'developer_instructions="Call functions.orbix__change_title to set title."'
            ]);
        });
