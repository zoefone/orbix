...
function isOrbixBridgeElicitation(params: unknown): boolean {
    const record = asRecord(params);
    return record?.serverName === 'orbix';
}
...
export function registerAppServerPermissionHandlers(args: {
    ...
    client.registerRequestHandler('mcpServer/elicitation/request', async (params) => {
        ...
        const shouldAccept = isOrbixBridgeElicitation(params) || currentPermissionMode === 'yolo';
