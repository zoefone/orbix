...
describe('registerAppServerPermissionHandlers', () => {
    ...
    it('accepts non-ORBIX MCP elicitation requests when live permission mode is yolo', async () => {
        const { client, handlers } = createClient();
    ...
    it('does not auto-accept non-ORBIX MCP elicitation requests in safe-yolo mode', async () => {
        const { client, handlers } = createClient();
        const permissionHandler = {
            handleToolCall: vi.fn()
        };
    ...
    it('cancels non-ORBIX MCP elicitation requests', async () => {
        const { client, handlers } = createClient();
        const permissionHandler = {
            handleToolCall: vi.fn()
        };
