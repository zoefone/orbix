...
import { buildOrbixMcpBridge } from './utils/buildOrbixMcpBridge';
...
type HappyServer = Awaited<ReturnType<typeof buildOrbixMcpBridge>>['server'];
...
class CodexRemoteLauncher extends RemoteLauncherBase {
    ...
    protected async runMainLoop(): Promise<void> {
        ...
        const isOrbixChangeTitleToolName = (toolName: string | null): boolean => {
            return toolName === 'mcp__orbix__change_title';
        };
        ...
        const handleChildCodexEvent = (agentId: string, msg: Record<string, unknown>): void => {
            ...
            if (msgType === 'mcp_tool_call_begin') {
                ...
                if (callId && name) {
                    ...
                    if (isOrbixChangeTitleToolName(name) && requestedTitle) {
        ...
        const { server: happyServer, mcpServers } = await buildOrbixMcpBridge(session.client, {
            ...
            // leak into the parent ORBIX session. Defer the side effect until
