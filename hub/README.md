...
Telegram bot + HTTP API + realtime updates for orbix hub.
...
- `ORBIX_PUBLIC_URL` - Public HTTPS URL for Telegram Mini App access. Also used to derive default CORS origins for the web app.
...
- `ORBIX_LISTEN_HOST` - HTTP bind address (default: 127.0.0.1).
- `ORBIX_LISTEN_PORT` - HTTP port (default: 3006).
...
export ORBIX_PUBLIC_URL="https://your-domain.example"
...
To enable Telegram, set TELEGRAM_BOT_TOKEN and ORBIX_PUBLIC_URL, start the hub, open `/app`
...
- Telegram Mini Apps require HTTPS and a public URL. If the hub has no public IP, use Cloudflare Tunnel or Tailscale and set `ORBIX_PUBLIC_URL` to the HTTPS endpoint.
- If the web app is hosted on a different origin, set `CORS_ORIGINS` (or `ORBIX_PUBLIC_URL`) to include that static host origin.
...
2. Set `CORS_ORIGINS` (or `ORBIX_PUBLIC_URL`) to the static host origin.
3. Open the static site, click the Hub button on the login screen, and enter the orbix hub origin.
