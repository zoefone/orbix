...
import type { Session, SyncEvent } from '@orbix/protocol/types'
...
describe('MessageService.releaseMatureScheduledMessages', () => {
    ...
    it('#10 hub cold-start restart: mature message is re-emitted by new Store+Service (true restart sim)', () => {
        const dir = mkdtempSync(join(tmpdir(), 'orbix-restart-test-'))
...
// ---------------------------------------------------------------------------
// ORBIX Bot R4: session-end sweep must not stamp mature scheduled rows
// ---------------------------------------------------------------------------
