/**
 * Sync Engine for ORBIX Telegram Bot (Direct Connect)
 *
 * In the direct-connect architecture:
 * - orbix-hub is the hub (Socket.IO + REST)
 * - orbix CLI connects directly to the hub (no relay)
 * - No E2E encryption; data is stored as JSON in SQLite
 */

import { isKnownFlavor, type LocalResumeTarget, type ResumableSession } from '@orbix/protocol'
import type { CursorMigrateOutcome, CursorMigrateToAcpRequest, SlashCommandsResponse } from '@orbix/protocol/apiTypes'
import type { AgentFlavor, CodexCollaborationMode, DecryptedMessage, PermissionMode, Session, SyncEvent } from '@orbix/protocol/types'
import { unwrapRoleWrappedRecordEnvelope } from '@orbix/protocol/messages'
import type { Server } from 'socket.io'
import type { Store, CancelQueuedMessageResult } from '../store'
import type { OrbixSessionExportResult } from '@orbix/protocol/sessionExport'
...
export type { Session, SyncEvent } from '@orbix/protocol/types'
...
export class SyncEngine {
    ...
    getSessionExport(sessionId: string, session: Session): OrbixSessionExportResult {
    ...
    /**
     * Apply the post-migration metadata flip in orbix.db:
     ...
     * succeeds. Kept on the engine (not on the migrator) so that all orbix.db
    ...
    private buildMigratorForRequest(_request: CursorMigrateToAcpRequest): CursorLegacyMigrator {
        ...
        return new CursorLegacyMigrator(migratorOpts, {
            ...
            // tiann/hapi#872: size sanity check needs to compare ORBIX's known
            ...
            getOrbixMessageCount: (sessionId, _namespace) => {
                try {
     ...
     *
     * The migrator's verify probe runs in an isolated ORBIX_HOME (see
    ...
    private async maybeAutoMigrateLegacyCursorSession(session: Session, namespace: string): Promise<Session> {
        ...
        const flagRaw = process.env.ORBIX_CURSOR_LEGACY_AUTO_MIGRATE?.trim().toLowerCase() ?? ''
