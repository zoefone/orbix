...
export async function loadServerSettings(dataDir: string): Promise<ServerSettingsResult> {
    ...
    if (process.env.ORBIX_LISTEN_HOST) {
        listenHost = process.env.ORBIX_LISTEN_HOST
    ...
    // listenPort: env > file > default
    let listenPort = 3006
    if (process.env.ORBIX_LISTEN_PORT) {
        const parsed = parseInt(process.env.ORBIX_LISTEN_PORT, 10)
        if (!Number.isFinite(parsed) || parsed <= 0) {
            throw new Error('ORBIX_LISTEN_PORT must be a valid port number')
        }
        listenPort = parsed
    ...
    if (process.env.ORBIX_PUBLIC_URL) {
        publicUrl = process.env.ORBIX_PUBLIC_URL
