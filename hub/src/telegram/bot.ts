...
/**
 * ORBIX Telegram Bot - Notification-only mode
 */
export class HappyBot implements NotificationChannel {
    ...
    async start(): Promise<void> {
        ...
        console.log('[ORBIXBot] Starting Telegram bot...')
        this.isRunning = true

        // Start polling (long-running, resolves when polling stops)
        this.bot.start({
            onStart: (botInfo) => {
                console.log(`[ORBIXBot] Bot @${botInfo.username} started`)
            }
        }).catch((error) => {
            this.isRunning = false
            console.error('[ORBIXBot] Telegram bot polling failed:', error instanceof Error ? error.message : error)
        })
    }
    ...
    async stop(): Promise<void> {
        ...
        console.log('[ORBIXBot] Stopping Telegram bot...')
    ...
    private setupMiddleware(): void {
        ...
        this.bot.catch((err) => {
            console.error('[ORBIXBot] Error:', err.message)
        })
    }
    ...
    private setupCommands(): void {
        ...
        this.bot.command('app', async (ctx) => {
            ...
            await ctx.reply('Open ORBIX Mini App:', { reply_markup: keyboard })
        })
    ...
    async sendReady(session: Session): Promise<void> {
        ...
        for (const chatId of chatIds) {
            try {
                ...
                console.error(`[ORBIXBot] Failed to send ready notification to chat ${chatId}:`, error)
            }
        }
    }
    ...
    async sendPermissionRequest(session: Session): Promise<void> {
        ...
        for (const chatId of chatIds) {
            try {
                ...
                console.error(`[ORBIXBot] Failed to send notification to chat ${chatId}:`, error)
            }
        }
    }
    ...
    async sendTaskNotification(session: Session, notification: TaskNotification): Promise<void> {
        ...
        for (const chatId of chatIds) {
            try {
                ...
                console.error(`[ORBIXBot] Failed to send task notification to chat ${chatId}:`, error)
            }
        }
    }
}
