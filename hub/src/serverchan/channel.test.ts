...
describe('ServerChanChannel', () => {
    it('does not send completed task notifications', async () => {
        ...
        try {
            const channel = new ServerChanChannel('SCT_TEST', 'https://orbix.example.com')
    ...
    it('sends failed task notifications', async () => {
        ...
        try {
            ...
            expect((init?.body as URLSearchParams).get('title')).toBe('ORBIX Task failed')
        } finally {
            globalThis.fetch = originalFetch
        }
    })
    ...
    it('sends session completion notifications', async () => {
        ...
        try {
            ...
            const init = call?.[1] as RequestInit | undefined
            expect(String(url)).toContain('https://sctapi.ftqq.com/SCT_TEST.send')
            expect((init?.body as URLSearchParams).get('title')).toBe('ORBIX Session completed')
        } finally {
            globalThis.fetch = originalFetch
        }
    })
