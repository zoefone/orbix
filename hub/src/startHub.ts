...
export async function startHub(options: StartHubOptions = {}): Promise<HubInstance> {
    console.log('ORBIX Hub starting...')
    ...
    console.log(`[Hub] ORBIX_LISTEN_HOST: ${config.listenHost} (${formatSource(config.sources.listenHost)})`)
    console.log(`[Hub] ORBIX_LISTEN_PORT: ${config.listenPort} (${formatSource(config.sources.listenPort)})`)
    console.log(`[Hub] ORBIX_PUBLIC_URL: ${config.publicUrl} (${formatSource(config.sources.publicUrl)})`)
    ...
    if (relayFlag.enabled) {
        tunnelManager = new TunnelManager({
            ...
            authKey: process.env.ORBIX_RELAY_AUTH || null,
            useRelay: process.env.ORBIX_RELAY_FORCE_TCP === 'true' || process.env.ORBIX_RELAY_FORCE_TCP === '1'
        })
    ...
    console.log('')
    console.log('ORBIX Hub is ready!')
