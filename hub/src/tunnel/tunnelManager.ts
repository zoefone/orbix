...
import { APP_VERSION } from '@orbix/protocol'

function getOrbixHome(): string {
    return process.env.ORBIX_HOME
        ? process.env.ORBIX_HOME.replace(/^~/, homedir())
        : join(homedir(), '.orbix')
}
...
function getTunwgPath(): string {
    ...
    if (isBunCompiled()) {
        const orbixHome = getOrbixHome()
        const runtimePath = join(orbixHome, 'runtime', APP_VERSION)
...
export class TunnelManager {
    ...
    private async spawnTunwg(): Promise<string | null> {
        ...
        if (!env.TUNWG_PATH) {
            env.TUNWG_PATH = join(getOrbixHome(), 'tunwg')
        }

        if (this.config.apiDomain) {
            env.TUNWG_API = this.config.apiDomain
        }
        env.TUNWG_AUTH = this.config.authKey ?? 'orbix'
