...
React Mini App / PWA for monitoring and controlling orbix sessions.
...
- `ORBIX_PUBLIC_URL` to the public HTTPS URL of the dev server.
...
The built assets land in `web/dist` and are served by orbix-hub. The single executable can embed these assets.

## Standalone hosting

You can host `web/dist` on a static host (GitHub Pages, Cloudflare Pages) and point it at any orbix hub:
...
3. Set hub CORS to allow the static origin (`ORBIX_PUBLIC_URL` or `CORS_ORIGINS`).
4. Open the static site, click the top-right Hub button on the login screen, and enter the orbix hub origin.
