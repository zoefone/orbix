...
import { VOICE_BACKEND_LABELS } from '@orbix/protocol/voicePickerCatalog'
...
export default function SettingsPage() {
    ...
    const [voiceLanguage, setVoiceLanguage] = useState<string | null>(() => {
        return localStorage.getItem('orbix-voice-lang')
    })
    ...
    const [voiceOpening, setVoiceOpening] = useState<'greet' | 'brief'>(() => {
        return localStorage.getItem('orbix-voice-proactive') === 'true' ? 'brief' : 'greet'
    })

    const handleVoiceOpeningChange = (value: 'greet' | 'brief') => {
        setVoiceOpening(value)
        if (value === 'brief') {
            localStorage.setItem('orbix-voice-proactive', 'true')
        } else {
            localStorage.removeItem('orbix-voice-proactive')
        }
    }
    ...
    const handleVoiceLanguageChange = (language: Language) => {
        ...
        if (language.code === null) {
            localStorage.removeItem('orbix-voice-lang')
        } else {
            localStorage.setItem('orbix-voice-lang', language.code)
        }
