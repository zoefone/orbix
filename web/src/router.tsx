...
function SessionsPage() {
    ...
    const normalizeCodexScriptError = useCallback((message: string | null | undefined, fallback: string): string => {
        ...
        if (/当前会话仍处于活跃状态，请等待会话结束后重试|Active Orbix process already has this Codex thread/i.test(raw)) {
    ...
    const handleImportCodexSessions = useCallback(async (sessionIds: string[]) => {
        ...
        try {
            // 中文注释：弹窗提交的是本地 Codex thread ID；后端会直接读取这些 transcript 并导入到 Orbix。
    ...
    return (
        <>
            ...
            {/* 中文注释：这里展示的是本地 Codex transcript 列表；默认尝试勾选当前 Orbix 会话关联的 Codex thread。 */}
...
function SessionPage() {
    ...
    const {
        ...
        onSuccess: (sentSessionId) => {
            ...
            // 中文注释：一旦用户已经在 Orbix 内继续这个 Codex 会话，就清除"刚从 Codex 导入"的标记。
...
function NewSessionPage() {
    ...
    const handleChooseFolder = useCallback((args: { machineId: string | null; directory: string }) => {
        ...
        // it rather than falling back to `orbix:lastMachineId`, which can
