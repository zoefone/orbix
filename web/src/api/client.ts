...
import type {
    ...
    UploadFileResponse
} from '@orbix/protocol/apiTypes'
import type { AgentFlavor } from '@orbix/protocol'
import type { CancelMessageResponse } from '@orbix/protocol/schemas'
...
export class ApiClient {
    ...
    async syncCodexSession(payload?: CodexDesktopSyncRequest): Promise<CodexDesktopScriptResponse> {
        // 中文注释：当前按钮语义已改为“从 Codex 导入到 Orbix”；这里提交的是本地 transcript 对应的 Codex thread ID 列表。
    ...
    async getSessionExport(sessionId: string, options?: { signal?: AbortSignal }): Promise<OrbixSessionExport> {
        return await this.request<OrbixSessionExport>(
