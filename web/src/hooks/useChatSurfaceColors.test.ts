...
describe('useChatSurfaceColors', () => {
    ...
    it('falls back to default when storage is missing or invalid', () => {
        localStorage.setItem('orbix-tool-group-bg', 'preset:invalid')
        localStorage.setItem('orbix-user-message-bg', 'custom:not-a-color')
    ...
    it('stores preset and custom preferences using stable string values', () => {
        ...
        expect(localStorage.getItem('orbix-tool-group-bg')).toBe('preset:soft-blue')
        expect(localStorage.getItem('orbix-user-message-bg')).toBe('custom:#88cc44')
    ...
    it('reapplies stored values during initialization', () => {
        localStorage.setItem('orbix-tool-group-bg', 'preset:soft-yellow')
        localStorage.setItem('orbix-user-message-bg', 'custom:#88cc44')
