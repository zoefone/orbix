...
describe('useSessionPreviewLimit helpers', () => {
    ...
    it('falls back to the default for missing or invalid values', () => {
        ...
        window.localStorage.setItem('orbix-session-preview-limit', 'invalid')
        expect(getInitialSessionPreviewLimit()).toBe(DEFAULT_SESSION_PREVIEW_LIMIT)

        window.localStorage.setItem('orbix-session-preview-limit', '12.5')
        expect(getInitialSessionPreviewLimit()).toBe(DEFAULT_SESSION_PREVIEW_LIMIT)
    })

    it('reads valid stored values', () => {
        window.localStorage.setItem('orbix-session-preview-limit', '12')
    ...
    it('clamps values to the supported range', () => {
        ...
        window.localStorage.setItem('orbix-session-preview-limit', '0')
        expect(getInitialSessionPreviewLimit()).toBe(MIN_SESSION_PREVIEW_LIMIT)

        window.localStorage.setItem('orbix-session-preview-limit', '120')
