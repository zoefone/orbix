...
describe('NewSession preferences', () => {
    ...
    it('loads saved values from storage', () => {
        localStorage.setItem('orbix:newSession:agent', 'codex')
        localStorage.setItem('orbix:newSession:yolo', 'true')
    ...
    it('falls back to default agent on invalid stored value', () => {
        localStorage.setItem('orbix:newSession:agent', 'unknown-agent')
    ...
    it('persists new values to storage', () => {
        ...
        expect(localStorage.getItem('orbix:newSession:agent')).toBe('gemini')
        expect(localStorage.getItem('orbix:newSession:yolo')).toBe('true')
    })
})
